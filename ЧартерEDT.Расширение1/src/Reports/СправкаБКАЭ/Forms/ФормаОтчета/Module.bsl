...
Процедура СформироватьНаСервере()
	...
	|	Accommodations.Room.ТипКомнаты AS ТипКвартиры,
	...
	|	Accommodations.Guest.КатегорияПроживающего AS КатегорияПроживающего,
	|	Isnull(Accommodations.Занят, 0) AS Занят,
	|	isnull(Accommodations.всего, 0) AS Всего,
	|	Accommodations.Guest.Отсутствие AS Отсутствие,
	|	Accommodations.Guest.ТипОтсутствия AS ТипОтсутствия,
	|	Accommodations.Guest.ОтсутствиеС AS ОтсутствиеС,
	|	Accommodations.Guest.ОтсутствиеПо AS ОтсутствиеПо
	|FROM
	...
	Пока ВыборкаГруппировка.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НомерКвартиры <> "" Тогда
				...
				Если НомерКвартиры <> Выборка.Квартира Тогда
					...
					ТабДок.Область("R"+Формат(НачальнаяСтрока,"NG=")+"C5:R"+Формат(КонечнаяСтрока,"NG=")+"C5").Объединить();
					...
					ТД.Область("R"+Формат(НачальнаяСтрокаТД,"NG=")+"C5:R"+Формат(КонечнаяСтрокаТД,"NG=")+"C5").Объединить();
			...
			Область.Параметры.ТипКвартиры = Выборка.ТипКвартиры;
			...
			Область.Параметры.КатегорияПроживающего = Выборка.КатегорияПроживающего;
			
			Если ЗначениеЗаполнено(Выборка.СотрудникПолноеИмя) Тогда
				Если НЕ Выборка.ТипОтсутствия = Перечисления.ТипОтсутствия.ПустаяСсылка() Тогда
					Область.Параметры.Отсутствие = Строка(Выборка.ТипОтсутствия) + " / " +Формат(Выборка.ОтсутствиеС, "ДФ=dd.MM.yy")
													+ " - "+ Формат(Выборка.ОтсутствиеПо, "ДФ=dd.MM.yy");
				Иначе
					Область.Параметры.Отсутствие = "";								
				КонецЕсли;
			КонецЕсли;
		...
		ТабДок.Область("R"+Формат(НачальнаяСтрока,"NG=")+"C5:R"+Формат(ТабДок.ВысотаТаблицы,"NG=")+"C5").Объединить();
		...
		ТД.Область("R"+Формат(НачальнаяСтрокаТД,"NG=")+"C5:R"+Формат(ТД.ВысотаТаблицы,"NG=")+"C5").Объединить();
