...
Procedure ЗаполнитьКвотыНаСервере()
	...
	Запрос.УстановитьПараметр("ДатаРейса", НачалоМесяца(Объект.ДатаРейса));
	Запрос.УстановитьПараметр("ТипКомпановки", Объект.ТипВС);
	
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		НоваяСтрока = Объект.Квотирование.Добавить();
