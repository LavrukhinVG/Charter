...
Процедура СформироватьБилетНаРейсДляСотрудника(ШаблонБилета, СтруктураПосадочногоТалона) Экспорт
	...
	Макет = ПолучитьОбщийМакет(чартеры_ОбщиеПроцедурыИФункцииСервер.ПолучитьШаблонПосадочногоТалона(КлассМестаВС));
	...
	Если НЕ Макет = Неопределено Тогда
		...
		Запрос = Новый Запрос;
		Запрос.Текст = ПолучитьТекстЗапросаПосадочногоТалона();
...
Функция ПолучитьТекстЗапросаПосадочногоТалона()
